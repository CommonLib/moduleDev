...
public abstract class InfoFragment<T extends ActivityPresenter> extends BaseFragment<T> {
    @BindView(R.id.info_sv_root)
    NestedScrollView mInfoSvRoot;
    @BindView(R.id.info_ll_root)
    LinearLayout mInfoLlRoot;
    ...
    @BindView(R.id.info_tv_graduate_school)
    TextView mInfoTvGraduateSchool;
    ...
    @BindView(R.id.info_tv_marital_status)
    TextView mInfoTvMaritalStatus;
    @BindView(R.id.info_tv_language)
    TextView mInfoTvLanguage;
    @BindView(R.id.info_tv_english_language)
    TextView mInfoTvEnglishLanguage;
    @BindView(R.id.info_tv_japanese_language)
    TextView mInfoTvJapaneseLanguage;
    @BindView(R.id.info_tv_language_others)
    TextView mInfoTvLanguageOthers;
    ...
    protected void initView() {
        ...
        showHeader(header);
        mInfoSvRoot.setOnScrollChangeListener(new NestedScrollView.OnScrollChangeListener() {
            @Override
            public void onScrollChange(NestedScrollView v, int scrollX, int scrollY, int oldScrollX,
                                       int oldScrollY) {
                if (WindowUtils.isSoftKeyboardShow()) {
                    WindowUtils.hideSoftKeyboard(v);
                    mInfoLlRoot.requestFocus();
                }
            }
        });
    }

    public void showDateTimePicker(DatePickerDialog.OnDateSetListener listener, long maxDate,
                                   long minDate) {// 获取当前的年、月、日、小时、分钟
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            mDatePickerDialog = new DatePickerDialog(getContext(), R.style.date_picker_dialog,
    ...
    public void setRedColor(IssueView issueView) {
        if (issueView.issueViews != null) {
            for (View view : issueView.issueViews) {
                if (view instanceof EditText) {
                    view.setBackgroundResource(R.drawable.shape_bg_info_edit_text_red);
                } else if (view instanceof TextView) {
                    ((TextView) view).setTextColor(getColor(R.color.red));
    ...
    public void setBlackColor(IssueView issueView) {
        if (issueView.issueViews != null) {
            for (View view : issueView.issueViews) {
                if (view instanceof EditText) {
                    view.setBackgroundResource(R.drawable.shape_bg_info_edit_text_grey);
                } else if (view instanceof TextView) {
                    ((TextView) view).setTextColor(getColor(R.color.gray_dark));
                } else if (view instanceof RadioGroup) {
                    RadioGroup rg = (RadioGroup) view;
                    int childCount = rg.getChildCount();
                    for (int i = 0; i < childCount; i++) {
                        ...
                        if (child instanceof RadioButton) {
                            RadioButton rb = (RadioButton) child;
                            rb.setTextColor(getColor(R.color.gray_dark));
                        }
                    }
                }
            }
    ...
    public IssueView checkEmpty() {
        ...
        String graduateSchool = mInfoEtGraduateSchool.getText().toString().trim();
        if (TextUtils.isEmpty(graduateSchool)) {
            issueEditText = mInfoEtGraduateSchool;
            issueTextView = mInfoTvGraduateSchool;
            return new IssueView(null, issueTextView, issueEditText);
        }
        ...
        if (!hasGender) {
            ...
            return new IssueView(issueRG, null, null);
        }

        boolean hasMaritalStatus = isRadioGroupSelected(mInfoRgMaritalStatus);
        if (!hasMaritalStatus) {
            issueRG = mInfoRgMaritalStatus;
            issueTextView = mInfoTvMaritalStatus;
            return new IssueView(issueRG, issueTextView, null);
        }
        ...
        if (!hasAcademicDegree) {
            ...
            return new IssueView(null, issueTextView, null);
        }

        boolean hasEnglishLanguage = isRadioGroupSelected(mInfoRgEnglishLanguage);
        String mLanguageOthers = mInfoEtLanguageOthers.getText().toString().trim();
        boolean hasJapaneseLanguage = isRadioGroupSelected(mInfoRgJapaneseLanguage);
        if (!hasEnglishLanguage && TextUtils.isEmpty(mLanguageOthers) && !hasJapaneseLanguage) {
            return new IssueView(null, mInfoTvLanguage, mInfoRgEnglishLanguage,
                    mInfoTvEnglishLanguage, mInfoRgJapaneseLanguage, mInfoTvJapaneseLanguage,
                    mInfoEtLanguageOthers, mInfoTvLanguageOthers);
        }
    ...
    public Candidate getCandidate() {
        ...
        candidate.basic.emergency = new Candidate.Contact(getEditText(mInfoEtEmergencyContact),
                getEditText(mInfoEtRelationship), getEditText(mInfoEtTelephoneNo));
    ...
    public void setCandidate(Candidate candidate) {
        ...
        } else if (TextUtils.equals(academicDegree, "本科") || TextUtils
                .equals(academicDegree, "Bachelor")) {
            mInfoAcademicDegree.check(R.id.info_rb_academic_degree_bachelor);
        } else if (TextUtils.equals(academicDegree, "硕士") || TextUtils
                .equals(academicDegree, "Master")) {
            mInfoAcademicDegree.check(R.id.info_rb_academic_degree_master);
        } else if (TextUtils.equals(academicDegree, "博士") || TextUtils
                .equals(academicDegree, "Doctorate")) {
        ...
        if (TextUtils.equals(recruitChannel, "猎头") || TextUtils
                .equals(recruitChannel, "Headhunting")) {
        ...
        } else if (TextUtils.equals(recruitChannel, "员工推荐") || TextUtils
                .equals(recruitChannel, "Employee Referral")) {
    ...
    public WorkExperienceView addExperienceLabelView() {
        View divideLine = inflateView(R.layout.view_divide_line_add_experience,
                mInfoLlWorkExperiences);
